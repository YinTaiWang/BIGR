...
#         ----------
#         I and J : (1, c, d, h, w)
#             Torch tensor of same shape. The number of channel (c) can be different, in which broadcasting will be used. 
#         windows_size : int
#             Side length of the square window to calculate the local NCC. 
...
#         assert ndims == 3, 'Currently only 3D is possible, found: %d' % ndims
...
#         I_sum = conv_fn(I, sum_filt, padding=padding)
#         J_sum = conv_fn(J, sum_filt, padding=padding)
#         I2_sum = conv_fn(I2, sum_filt, padding=padding)
#         J2_sum = conv_fn(J2, sum_filt, padding=padding)
#         IJ_sum = conv_fn(IJ, sum_filt, padding=padding)

#         win_size = np.prod(win)
#         u_I = I_sum / win_size
#         u_J = J_sum / win_size
        
#         cross = (IJ_sum - u_I * u_J * win_size) / win_size
#         I_var = I2_sum/win_size - u_I**2
#         J_var = J2_sum/win_size - u_J**2
        
#         cc = (cross + self.smooth_nr) / (I_var * J_var)**0.5 + self.smooth_dr
