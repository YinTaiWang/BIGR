...
def main():
    ...
    print("-"*30)
    print("\n## Settings -- BLT_baseline (Binary)")
    print(f"Files will be save at: baseline_binary/{attempt}")
    ...
    model_folder = os.path.join(script_dir, "..", "data", "BLT_radiomics", "models", "baseline_binary")
    ...
    for fold, (train_val_idx, test_idx) in enumerate(skf.split(images, labels)):
        ...
        train_idx, val_idx = train_test_split(indices, test_size=0.25, random_state=random_state)
        ...
        plotting.save_progress(train_history_cv, val_history_cv, model_dir)
    ...
    plotting.plotting(ALL_y_ts, ALL_y_ts_bm_pred_argmax, ALL_y_ts_bm_pred, model_name="best", save_path=fig_path)
    plotting.plotting(ALL_y_ts, ALL_y_ts_fm_pred_argmax, ALL_y_ts_fm_pred, model_name="final", save_path=fig_path)
